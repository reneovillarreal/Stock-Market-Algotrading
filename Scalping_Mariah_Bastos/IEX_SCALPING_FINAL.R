...
# Tiingo Token
riingo_set_token("3c0aef2d84350cebf792c29fa361a5b5539059e6")
...
repeat 
{
  if((format(Sys.time(), format = "%H:%M:%S") >= "13:00:00") & (format(Sys.time(), format = "%H:%M:%S") < "14:30:00"))
  {
    ...
    Budget <- (fromJSON("https://bigdataforall.com/stocks/?key=barthez&tiingo_key=3c0aef2d84350cebf792c29fa361a5b5539059e6&request=profile")$budget)
    ...
    
  {
    ...
    for (b in Symbol_List)
    {
      ...
      # Creating simple moving average (Scalping) lists. See ReadMe for details.
      Scalping_Total[[b]] <- Symbol_History_Final %>%
        filter(ticker == b) %>%
        mutate(price_avg = close) %>%
        select(ticker, date, price_avg) %>%
        group_by(ticker) %>%
        arrange(date) %>%
        mutate(current_price = price_avg) %>%
        mutate(previous_price = lag(price_avg, order_by = date))
        filter(date >= (Sys.Date() - 4)) %>%
        tail(1) %>%
        mutate(action = ifelse((current_price > previous_price), "SELL", "BUY"))
                       
    }
    
    # Binding Scalping lists.
    Scalping_Total_Final <- unique(do.call("rbind", Scalping_Total)) %>%
      filter((action == "BUY") | (action == "SELL")) %>%
      arrange(date)
    
    Scalping_Total_Final[nrow(Scalping_Total_Final)+1,] <- NA
    
    # Generating a count of how many signals the model generated in one minute
    Signals_per_minute <- select(Scalping_Total_Final, date, action) %>%
      select(date) %>%
      group_by(date) %>%
      summarize(count = n()) %>%
      tail(1)
    ...
    Master_Log <- fromJSON("https://bigdataforall.com/stocks/?key=barthez&tiingo_key=3c0aef2d84350cebf792c29fa361a5b5539059e6&request=log")$log%>%
    ...
    if (Signals_per_minute$count[1] <= 2 | is.na(Signals_per_minute$count[1]))
    {
      ...
      for (c in (1:nrow(Scalping_Total_Final))) 
      {
        
        sym <- Scalping_Total_Final$ticker[c]
        ...
        if((ticker_count$count[1] < 10) | (is.na(ticker_count$count[1])))
        {
          ...
          if(!is.na(Scalping_Total_Final$ticker[c]))
          {
            
            # Signal = Buy
            if(Scalping_Total_Final$action[c] == "BUY")
            {
              
              # Comparing budget to price of 2 stocks
              if(Budget >= (Scalping_Total_Final$price_avg[c]*2))
              {
                url_buy_base <- "https://bigdataforall.com/stocks/?key=barthez&tiingo_key=3c0aef2d84350cebf792c29fa361a5b5539059e6&request=buy"
                quantity_buy <- c(2)
                ...
                buy_sucess_log <- fromJSON("https://bigdataforall.com/stocks/?key=barthez&tiingo_key=3c0aef2d84350cebf792c29fa361a5b5539059e6&request=log")$log%>%
                  select(symbol, request, quantity, result, details, date, budget) %>%
                  filter(request == "buy", result == "success")
                
                # Update budget
                budget_buy_log <- fromJSON("https://bigdataforall.com/stocks/?key=barthez&tiingo_key=3c0aef2d84350cebf792c29fa361a5b5539059e6&request=log")$log%>%
              ...
              # Signal = Sell
            } else if(Scalping_Total_Final$action[c] == "SELL")
            {
              ...
              if(sym %in% Master_Log$symbol)
              {
                ...
                url_sell_base <- "https://bigdataforall.com/stocks/?key=barthez&tiingo_key=3c0aef2d84350cebf792c29fa361a5b5539059e6&request=sell"
                quantity_sell <- c(1)
                ...
                sell_sucess_log <- fromJSON("https://bigdataforall.com/stocks/?key=barthez&tiingo_key=3c0aef2d84350cebf792c29fa361a5b5539059e6&request=log")$log%>%
                  select(symbol, request, quantity, result, details, date, budget) %>%
                  filter(request == "sell", result == "success")
                
                # Updating budget
                budget_sell_log <- fromJSON("https://bigdataforall.com/stocks/?key=barthez&tiingo_key=3c0aef2d84350cebf792c29fa361a5b5539059e6&request=log")$log%>%
                  mutate(id = as.integer(id), budget = as.double(budget)) %>%
                  select(id, budget) %>%
                  filter(id == max(id))$budget
    ...
    } else
    {
      ...
      Scalping_Total_Final <- Scalping_Total_Final %>%
        filter(action == 'BUY' | action == 'SELL')
      head(2)
      
      Scalping_Total_Final[nrow(Scalping_Total_Final)+1,] <- NA
      
      for (d in (1:nrow(Scalping_Total_Final))) 
      {
        
        sym <- Scalping_Total_Final$ticker[d]
        ...
        if((ticker_count$count < 10) | (is.na(ticker_count$count)))
        {
          ...
          if(!is.na(Scalping_Total_Final$ticker[d])){
            
            # Signal = Buy
            if(Scalping_Total_Final$action[d] == "BUY")
            {
              
              if(Budget >= Scalping_Total_Final$price_avg[d]*2)
              {
                url_buy_base <- "https://bigdataforall.com/stocks/?key=barthez&tiingo_key=3c0aef2d84350cebf792c29fa361a5b5539059e6&request=buy"
                quantity_buy <- c(2)
              ...
              # Signal = Sell
            } else if(Scalping_Total_Final$action[d] == "SELL")
            {
    ...
    # Updating budget
    Budget <- fromJSON("https://bigdataforall.com/stocks/?key=barthez&tiingo_key=3c0aef2d84350cebf792c29fa361a5b5539059e6&request=log")$log%>%
      mutate(id = as.integer(id), budget = as.double(budget)) %>%
      select(id, budget) %>%
      filter(id == max(id))$budget
