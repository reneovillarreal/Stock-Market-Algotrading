...
repeat 
{
  if((format(Sys.time(), format = "%H:%M:%S") >= "13:00:00") & (format(Sys.time(), format = "%H:%M:%S") < "14:30:00"))
  {
    ...
    Budget <- (fromJSON("https://bigdataforall.com/stocks/?key=blanc&tiingo_key=436c2f794016aa2e308bc04cc84b49bff63ac9dd&request=profile")$budget)
    ...
    
  {
    ...
    for (b in Symbol_List)
    {
      ...
      # Creating exponential moving average (EMA) lists. See ReadMe for details.
      EMA_Total[[b]] <- Symbol_History_Final %>%
        filter(ticker == b) %>%
        mutate(price_avg = close) %>%
        select(ticker, date, price_avg) %>%
        group_by(ticker) %>%
        arrange(date) %>%
        mutate(avg_roll_30 = EMA(price_avg, 30)) %>%
        mutate(avg_roll_90 = EMA(price_avg, 90)) %>%
    ...
    # Binding EMA lists.
    EMA_Total_Final <- unique(do.call("rbind", EMA_Total)) %>%
      filter((action == "BUY") | (action == "SELL")) %>%
      arrange(date)
    
    EMA_Total_Final[nrow(EMA_Total_Final)+1,] <- NA
    
    # Generating a count of how mnay signals the model generated in one minute
    Signals_per_minute <- select(EMA_Total_Final, date, action) %>%
      select(date) %>%
      group_by(date) %>%
      summarize(count = n()) %>%
      tail(1)
    ...
    Master_Log <- fromJSON("https://bigdataforall.com/stocks/?key=blanc&tiingo_key=436c2f794016aa2e308bc04cc84b49bff63ac9dd&request=log")$log%>%
    ...
    if (Signals_per_minute$count[1] <= 2 | is.na(Signals_per_minute$count[1]))
    {
      ...
      for (c in (1:nrow(EMA_Total_Final))) 
      {
        
        sym <- EMA_Total_Final$ticker[c]
        ...
        if((ticker_count$count[1] < 10) | (is.na(ticker_count$count[1])))
        {
          ...
          if(!is.na(EMA_Total_Final$ticker[c]))
          {
            
            # Signal = Buy
            if(EMA_Total_Final$action[c] == "BUY")
            {
              
              # Comparing budget to price of 2 stocks
              if(Budget >= (EMA_Total_Final$price_avg[c]*2))
              {
                url_buy_base <- "https://bigdataforall.com/stocks/?key=blanc&tiingo_key=436c2f794016aa2e308bc04cc84b49bff63ac9dd&request=buy"
                quantity_buy <- c(2)
                ...
                buy_sucess_log <- fromJSON("https://bigdataforall.com/stocks/?key=blanc&tiingo_key=436c2f794016aa2e308bc04cc84b49bff63ac9dd&request=log")$log%>%
                  select(symbol, request, quantity, result, details, date, budget) %>%
                  filter(request == "buy", result == "success")
                
                # Update budget
                budget_buy_log <- fromJSON("https://bigdataforall.com/stocks/?key=blanc&tiingo_key=436c2f794016aa2e308bc04cc84b49bff63ac9dd&request=log")$log%>%
              ...
              # Signal = Sell
            } else if(EMA_Total_Final$action[c] == "SELL")
            {
              ...
              if(sym %in% Master_Log$symbol)
              {
                ...
                url_sell_base <- "https://bigdataforall.com/stocks/?key=blanc&tiingo_key=436c2f794016aa2e308bc04cc84b49bff63ac9dd&request=sell"
                quantity_sell <- c(1)
                ...
                sell_sucess_log <- fromJSON("https://bigdataforall.com/stocks/?key=blanc&tiingo_key=436c2f794016aa2e308bc04cc84b49bff63ac9dd&request=log")$log%>%
                  select(symbol, request, quantity, result, details, date, budget) %>%
                  filter(request == "sell", result == "success")
                
                # Updating budget
                budget_sell_log <- fromJSON("https://bigdataforall.com/stocks/?key=blanc&tiingo_key=436c2f794016aa2e308bc04cc84b49bff63ac9dd&request=log")$log%>%
                                      mutate(id = as.integer(id), budget = as.double(budget)) %>%
                                      select(id, budget) %>%
                                      filter(id == max(id))$budget
    ...
    } else
    {
      ...
      EMA_Total_Final <- EMA_Total_Final %>%
        filter(action == 'BUY' | action == 'SELL')
      head(2)
      
      EMA_Total_Final[nrow(EMA_Total_Final)+1,] <- NA
      
      for (d in (1:nrow(EMA_Total_Final))) 
      {
        
        sym <- EMA_Total_Final$ticker[d]
        ...
        if((ticker_count$count < 10) | (is.na(ticker_count$count)))
        {
          ...
          if(!is.na(EMA_Total_Final$ticker[d])){
            
            # Signal = Buy
            if(EMA_Total_Final$action[d] == "BUY")
            {
              
              if(Budget >= EMA_Total_Final$price_avg[d]*2)
              {
                url_buy_base <- "https://bigdataforall.com/stocks/?key=blanc&tiingo_key=436c2f794016aa2e308bc04cc84b49bff63ac9dd&request=buy"
                quantity_buy <- c(2)
              ...
              # Signal = Sell
            } else if(EMA_Total_Final$action[d] == "SELL")
            {
    ...
    # Updating budget
    Budget <- fromJSON("https://bigdataforall.com/stocks/?key=blanc&tiingo_key=436c2f794016aa2e308bc04cc84b49bff63ac9dd&request=log")$log%>%
                 mutate(id = as.integer(id), budget = as.double(budget)) %>%
                 select(id, budget) %>%
                 filter(id == max(id))$budget
